...
"""
BMP Echo - Simula TUTTI i parametri dell'eco di Audacity
Mappatura: delay time → delay (secondi), decay factor → decay (0.0-1.0)
...
# ======================================================================
# 2. ECO CON SOX (MAPPATURA PARAMETRI AUDACITY)
# ======================================================================
...
class SoxEcho:
    ...
    @staticmethod
    def apply_echo(input_path: Path, output_path: Path, fmt: str, 
                   gain_in: float, gain_out: float, delay: float, decay: float):
        """
        Applica eco con SoX usando l'effetto 'echos'.
        Audacity usa tipicamente 2-3 echi.
        Qui usiamo 3 echi per simulare l'eco di Audacity.
        """
        flags = SoxEcho._sox_format_flags(fmt)
        
        # Usa 'echos' con 3 ripetizioni
        # Parametri: gain-in gain-out delay decay delay decay delay decay
        cmd = [
            'sox', '-t', 'raw', '-r', '44100'] + flags + [str(input_path),
            '-t', 'raw', '-r', '44100'] + flags + [str(output_path),
            'echos', str(gain_in), str(gain_out), 
            str(delay), str(decay),
            str(delay), str(decay * 0.8),
            str(delay), str(decay * 0.5)
        ]
...
class BMPEcho:
    ...
    def _genera_livelli(self, seed: int, count: int = 50, fmt_key: str = '') -> List[Dict]:
        """Genera count livelli di eco con parametri DIVERSI per formato."""
        random.seed(seed)
        livelli = []
        
        # ===== Range DIFFERENTI per ogni formato =====
        if fmt_key == '8bit':
            # Effetti sottili: delay breve, decay basso
            delay_range = (0.05, 0.5)
            decay_range = (0.05, 0.3)
            gain_in_range = (0.7, 1.0)
            gain_out_range = (0.1, 0.5)
        elif fmt_key == '16bit':
            delay_range = (0.1, 1.5)
            decay_range = (0.1, 0.6)
            gain_in_range = (0.5, 0.9)
            gain_out_range = (0.3, 0.8)
        elif fmt_key == '24bit':
            delay_range = (0.2, 3.0)
            decay_range = (0.2, 0.8)
            gain_in_range = (0.3, 0.8)
            gain_out_range = (0.5, 1.0)
        elif fmt_key in ['32bit', '32bit_float']:
            delay_range = (0.5, 4.5)
            decay_range = (0.4, 0.9)
            gain_in_range = (0.2, 0.6)
            gain_out_range = (0.7, 1.0)
        elif fmt_key == '64bit_float':
            delay_range = (1.0, 5.0)
            decay_range = (0.5, 0.95)
            gain_in_range = (0.1, 0.4)
            gain_out_range = (0.8, 1.2)
        else:
            delay_range = (0.1, 2.0)
            decay_range = (0.1, 0.7)
            gain_in_range = (0.4, 0.8)
            gain_out_range = (0.3, 0.8)
        
        for _ in range(count):
            delay = random.uniform(delay_range[0], delay_range[1])
            decay = random.uniform(decay_range[0], decay_range[1])
            gain_in = random.uniform(gain_in_range[0], gain_in_range[1])
            gain_out = random.uniform(gain_out_range[0], gain_out_range[1])
            
            # 20% di probabilità di "eco estremo"
    ...
    def _write_params_txt(self, folder: Path, filename: str, params: Dict, 
        ...
        # Numero di echi (simulato con 3 ripetizioni in SoX)
        num_echi = 3
        
        with open(txt_path, 'w', encoding='utf-8') as f:
            f.write("=" * 50 + "\n")
            f.write("ECO - PARAMETRI AUDACITY\n")
            f.write("=" * 50 + "\n\n")
            ...
            f.write("PARAMETRI ECO:\n")
            f.write("  Ritardo temporale (s):   {:.2f}\n".format(params['delay_time']))
            f.write("  Fattore di decadimento:  {:.2f}\n".format(params['decay_factor']))
            f.write("  Numero di echi:          {:d}\n".format(num_echi))
            f.write("-" * 50 + "\n\n")
            
            f.write("PARAMETRI INTERNI (mapping SoX):\n")
            f.write("  gain-in (segnale originale):   {:.2f}\n".format(params['gain_in']))
            f.write("  gain-out (segnale eco):        {:.2f}\n".format(params['gain_out']))
            f.write("  delay (ritardo):               {:.3f}s\n".format(params['delay']))
            f.write("  decay (decadimento):           {:.3f}\n".format(params['decay']))
            f.write("  ripetizioni:                   3 (con decay progressivo)\n")
            f.write("\n" + "=" * 50 + "\n")
            f.write("COME RIPRODURRE IN AUDACITY:\n")
            f.write("=" * 50 + "\n")
            f.write("1. File → Importa → Dati Grezzi (Raw Data)\n")
            f.write("2. Scegli la codifica: {}\n".format(fmt))
            f.write("3. Effetto → Eco\n")
    ...
    def _process_single(self, fmt: str, folder: Path, params: Dict, 
        ...
        """Applica eco a tutto il file o a una sezione."""
        ...
        try:
            if sezione_start == 0 and sezione_end == 1:
                SoxEcho.apply_echo(raw_in, raw_out, fmt,
                ...
                if start < end and start >= 0 and end <= len(data):
                    ...
                    with tempfile.NamedTemporaryFile(suffix='.sec', delete=False) as f:
                        sec_out = Path(f.name)
                    SoxEcho.apply_echo(sec_in, sec_out, fmt,
                                      params['gain_in'], params['gain_out'],
                                      params['delay'], params['decay'])
        ...
        try:
            fname = f"{self.base_name}_{fmt}_echo_{suffix}.bmp"
    ...
    def _params_to_suffix(self, params: Dict) -> str:
        ...
        return (f"d{int(params['delay']*100):03d}"
                f"_dec{int(params['decay']*100):02d}"
                f"_gi{int(params['gain_in']*10):02d}"
                f"_go{int(params['gain_out']*10):02d}")
    ...
    def process(self):
        ...
        echo_folder = base_folder / "Eco"
        echo_folder.mkdir(parents=True, exist_ok=True)

        totali = 0

        for fmt_idx, (fmt_key, fmt_name) in enumerate(BMPConverter.FORMAT_NAMES.items()):
            fmt_folder = echo_folder / fmt_name
            ...
            with open(readme_path, 'w', encoding='utf-8') as rf:
                rf.write(f"ECO - {self.input_path.name}\n")
                rf.write(f"Formato: {fmt_name}\n")
                rf.write("=" * 60 + "\n")
                rf.write("Parametri Audacity mappati su SoX:\n")
                rf.write("  Delay time → delay (secondi)\n")
                rf.write("  Decay factor → decay (0.0-1.0)\n")
                rf.write("  Numero echi: 3 (con decay progressivo)\n")
        ...
        summary_readme = echo_folder / "README_COMPLETO.txt"
        with open(summary_readme, 'w', encoding='utf-8') as sf:
            sf.write(f"ECO - {self.input_path.name}\n")
            sf.write("=" * 60 + "\n")
            ...
            sf.write("  Delay time → delay (0.05-5.0 secondi)\n")
            sf.write("  Decay factor → decay (0.05-0.95)\n")
            sf.write("  Numero echi: 3 (come Audacity)\n")
            sf.write("\nOgni formato ha un set di parametri DIVERSO.\n")
            sf.write("\nFormati disponibili:\n")
            for fmt_name in BMPConverter.FORMAT_NAMES.values():
                sf.write(f"  • {fmt_name}\n")

        print(f"   ✅ Generati {totali} file eco per {self.base_name}")
...
def main():
    parser = argparse.ArgumentParser(
        description="BMP Echo - Simula TUTTI i parametri dell'eco di Audacity"
    )
    parser.add_argument('-i', '--input-dir', default='.',
                        help='Directory contenente i BMP (default: corrente)')
    parser.add_argument('-o', '--output-dir', default='bmp_echo_output',
                        help='Directory di output (default: bmp_echo_output)')
    ...
    print("⚡ ECO - TUTTI i parametri di Audacity mappati")
    ...
    for bmp_path in bmp_files:
        echo = BMPEcho(bmp_path, output_dir)
        echo.process()
